...
const Analytics = () => {
  ...
  // When a category is selected we keep bar heights but dim the income bar
  // (income has no category); pie slice opacity handles the pie.
  ...
  return (
    <DashboardLayout title="Detailed Analytics">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-3">
          ...
          <div className="relative">
            ...
            {isFilterOpen && (
              <div className="absolute top-full mt-2 w-56 bg-white border border-slate-100 rounded-2xl shadow-2xl z-50 py-2">
                {categories.map((cat) => (
                  <button
                    key={cat}
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-[#0A1128] text-white rounded-2xl text-xs font-bold hover:bg-slate-800 transition-all disabled:opacity-60 disabled:cursor-not-allowed active:scale-95"
          >
            <Download size={16} />
      ...
      {/* ── Error Banner ────────────────────────────────────────────────── */}
      {error && (
        <div className="mb-6 px-5 py-4 bg-red-50 border border-red-100 rounded-2xl text-xs font-bold text-red-600">
          ⚠️ {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-[#0A1128] rounded-full" />
                <span className="text-[10px] font-bold text-slate-400">Income</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-blue-500 rounded-full" />
                <span className="text-[10px] font-bold text-slate-400">Expenses</span>
              </div>
            </div>
          </div>
            ...
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  />
                  <Bar dataKey="income"  fill="#0A1128" radius={[6,6,0,0]} barSize={18}
        ...
        <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm">
          ...
          {isLoading ? (
            ...
            <div className="h-64 flex items-center justify-center text-slate-300 text-xs font-bold">No expense data.</div>
          ) : (
            <>
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            <h4 className="text-xl font-black text-slate-900">Income Trend</h4>
            <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
              savingsPositive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'
            }`}>
          ...
          <div className="h-[250px]">
            {isLoading ? (
              ...
              <div className="h-full flex items-center justify-center text-slate-300 text-xs font-bold">No data.</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-gradient-to-br from-[#0A1128] to-blue-900 rounded-[2.5rem] p-10 text-white shadow-xl flex flex-col justify-between group overflow-hidden relative">
          ...
          <button
            onClick={handleExport}
            disabled={isExporting}
            className="w-full py-4 bg-blue-500 hover:bg-blue-400 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all mt-8 relative z-10 active:scale-95 shadow-lg shadow-blue-900/50 disabled:opacity-60"
          >
